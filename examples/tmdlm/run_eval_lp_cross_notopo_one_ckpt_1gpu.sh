#!/usr/bin/env bash
# Single-checkpoint cross-dataset LP eval for Cora/PubMed targets.
#
# Usage:
#   sbatch --export=ALL,SOURCE_DATASET=cora,TARGET_DATASET=pubmed,CKPT_DIR=<path> \
#     examples/tmdlm/run_eval_lp_cross_notopo_one_ckpt_1gpu.sh
...
#SBATCH --job-name=lp-cross-1gpu
#SBATCH --output=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/lp_cross_1gpu_%j.log
#SBATCH --error=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/lp_cross_1gpu_%j.log
...
mkdir -p "$LOG_DIR"
LOG_JSONL="${LOG_JSONL:-${LOG_DIR}/eval_lp_cross_dataset_notopo_final.jsonl}"

EXP_TAG="lp_cross_${SOURCE_DATASET}_to_${TARGET_DATASET}_$(basename "${CKPT_DIR}")_${SLURM_JOB_ID:-local}"
