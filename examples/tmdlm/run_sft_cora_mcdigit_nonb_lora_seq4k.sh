...
# TM-DLM SFT on Cora, mc_digit, nonb (no neighbor labels), max_seq_len=4096.
# Mirrors run_sft_arxiv_mcdigit_nonb_lora.sh (seq=4096) but on cora with 7 classes.
#
# Usage:
#   GPUS=4,5 bash examples/tmdlm/run_sft_cora_mcdigit_nonb_lora_seq4k.sh
...
RUN_TAG="${RUN_TAG:-cora_$(date +%Y%m%d)_mcdigit_nonb_seq4k}"
