...
# One-shot PubMed eval for mc_digit SFT checkpoints WITHOUT neighbor labels.
# Counterpart to run_eval_pubmed_mcdigit_oneshot.sh — same setup but
# include_neighbor_labels=False, giving a fair comparison with baselines
# that don't receive oracle neighbor class info at test time.
#
# Usage:
#   GPU_ID=6 bash run_eval_pubmed_mcdigit_nolabel.sh
#   GPU_ID=7 bash run_eval_pubmed_mcdigit_nolabel.sh
#
...
MAX_ANSWER_TOKENS=1
INCLUDE_NEIGHBOR_LABELS=False   # key difference
...
WORKER_LOG="${WORK_LOG_ROOT}/eval-pubmed-mcdigit-nolabel-gpu${GPU_ID}.worker.log"
: > "${WORKER_LOG}"
...
echo "[$(ts)] [start] GPU=${GPU_ID} CKPTS=${CKPTS[*]} MAX_SAMPLES=${MAX_SAMPLES} include_neighbor_labels=False" >> "${WORKER_LOG}"
...
for job in "${JOBS[@]}"; do
  ...
  case "${kind}" in
    logit)
      script="${EVAL_LOGIT}"
      jsonl="${JSONL_ROOT}/eval-pubmed-2hop-${setting}-nolabel-${RUN_TAG}.jsonl"
      extra_args=( --position_id_type sequential --max_samples "${MAX_SAMPLES}" )
      ;;
    infill)
      script="${EVAL_INFILL}"
      jsonl="${JSONL_ROOT}/eval-pubmed-2hop-${setting}-nolabel-${RUN_TAG}-infill.jsonl"
  ...
  for step in "${CKPTS[@]}"; do
    ...
    exp="eval-pubmed-2hop-${setting}-nolabel-${kind}-${RUN_TAG}-checkpoint-${step}"
