...
# Unified TM-DLM SFT launcher for the nonb (no-neighbor-labels, fair-vs-LLaGA)
# setting on cora / pubmed / ogbn-arxiv. Runs topo + notopo in parallel on
# two GPUs. Supports both prompt formats: mc_digit (single-digit answer) and
# category_infill (full class-name infill).
#
# Usage:
#   GPUS=4,5 DATASET=cora    PROMPT_FORMAT=mc_digit       bash examples/tmdlm/run_sft_nonb_lora.sh
#   GPUS=4,5 DATASET=cora    PROMPT_FORMAT=category_infill bash examples/tmdlm/run_sft_nonb_lora.sh
#   GPUS=4,5 DATASET=pubmed  PROMPT_FORMAT=mc_digit       bash examples/tmdlm/run_sft_nonb_lora.sh
#   GPUS=4,5 DATASET=pubmed  PROMPT_FORMAT=category_infill bash examples/tmdlm/run_sft_nonb_lora.sh
#
# Optional env vars (defaults follow the format):
#   NUM_EPOCHS=10|20         (mc_digit defaults to 10, catinfill to 20)
#   MAX_ANSWER_TOKENS=1|6    (mc_digit -> 1, catinfill -> 6)
#   ANSWER_LABEL_STYLE=digit0   (mc_digit only)
#   RUN_TAG, OUTPUT_ROOT, USE_SRUN, EXTRA_ARGS, LR, BATCH_SIZE...
#
# Resume:
#   EXTRA_ARGS="--resume_from_checkpoint True"
#
# This extends/supersedes run_sft_cora_nonb_lora.sh,
# run_sft_cora_catinfill_nonb_lora.sh, run_sft_pubmed_nonb_lora.sh, and
# run_sft_pubmed_mcdigit_nonb_20260429.sh by parameterizing dataset + format.
...
# --- required ---
DATASET_NAME="${DATASET:-${DATASET_NAME:-cora}}"
PROMPT_FORMAT="${PROMPT_FORMAT:-mc_digit}"
case "${PROMPT_FORMAT}" in
  mc_digit|category_infill) : ;;
  *) echo "PROMPT_FORMAT must be mc_digit or category_infill, got: ${PROMPT_FORMAT}" >&2; exit 2 ;;
esac
...
if [[ "${PROMPT_FORMAT}" = "mc_digit" ]]; then
  : "${NUM_EPOCHS:=10}"
  : "${MAX_ANSWER_TOKENS:=1}"
  : "${ANSWER_LABEL_STYLE:=digit0}"
  format_tag="mcdigit-d0"
else
  : "${NUM_EPOCHS:=20}"
  : "${MAX_ANSWER_TOKENS:=6}"
  : "${ANSWER_LABEL_STYLE:=}"
  format_tag="catinfill"
fi

# --- shared training args ---
BASE_MODEL="${BASE_MODEL:-GSAI-ML/LLaDA-8B-Instruct}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/.models}"
# Sanitize DATASET_NAME for filesystem paths (comma-separated -> dash-joined)
...
for i in "${!EXPERIMENTS[@]}"; do
  ...
  if [[ -n "${ANSWER_LABEL_STYLE}" ]]; then
    cmd+=( --answer_label_style "${ANSWER_LABEL_STYLE}" )
  fi
  ...
  echo "[launch] dataset=${DATASET_NAME} format=${PROMPT_FORMAT} gpu=${gpu_id} topo=${use_topology_mask} ep=${NUM_EPOCHS} -> ${output_dir}"
  echo "[log] ${log_file}"
