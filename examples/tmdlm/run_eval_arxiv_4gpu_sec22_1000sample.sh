...
LOG_DIR=$WORK/logs
JSONL=$JSONL_DIR/sec22_fulltrain_1000.jsonl
...
launch_ckpt() {
  local GPU=$1 STEP=$2
  local EXP=sec22-fulltrain-1000-checkpoint-${STEP}
  local CKPT=$RUN_DIR/checkpoint-${STEP}
  local NPZ=$LOGITS_DIR/sec22_ckpt-${STEP}.npz
  local WLOG=$LOG_DIR/sec22-1000-gpu${GPU}-ckpt${STEP}.worker.log
  : > $WLOG
