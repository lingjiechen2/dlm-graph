#!/usr/bin/env bash
# PubMed NC SFT, no topology mask, 3 nodes x 8 H200.
#
# Mirrors run_sft_pubmed_nc_topo_3node.sh, but uses full causal attention
# with use_topology_mask=False and keeps at most 2 checkpoints.
...
#SBATCH --job-name=pubmed-nc-notopo
#SBATCH --output=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/pubmed_nc_notopo_keep2_%j.log
#SBATCH --error=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/pubmed_nc_notopo_keep2_%j.log
...
RUN_TAG="pubmed_nc_notopo_$(date +%Y%m%d_%H%M)_24gpu_10ep_mcdigit_d0_nonb_seq4k_keep2"
run_name="tmdlm-llada-8b-pubmed-nc-2hop-notopo-mcdigit-d0-nonb-r64-ep10-${RUN_TAG}"
...
echo "[launch] recipe=pubmed-only nc topo=False mc_digit digit0 nonb seq4k cudnn_sdp=${TMDLM_ENABLE_CUDNN_SDP}"
