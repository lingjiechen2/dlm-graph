#!/usr/bin/env bash
# Demo training script for PubMed.
...
if [ "${TASK}" = "nc" ]; then
  "${PYTHON}" "${COMMON_ARGS[@]}" \
    --task nc \
    --output_dir "${OUTPUT_ROOT}/pubmed_nc_notopo_r64_ep10_seq4k" \
    --run_name pubmed_nc_notopo_r64_ep10_seq4k \
  ...
  "${PYTHON}" "${COMMON_ARGS[@]}" \
    ...
    --lp_pos_weight 1.0 \
    --output_dir "${OUTPUT_ROOT}/pubmed_lp_llaga_topo_r64_ep5" \
    --run_name pubmed_lp_llaga_topo_r64_ep5 \
