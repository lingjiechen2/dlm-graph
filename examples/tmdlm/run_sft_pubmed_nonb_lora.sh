...
# Run TM-DLM SFT on PubMed WITHOUT neighbor labels (fair baseline comparison).
#
# Key difference from run_sft_pubmed_index_lora.sh:
#   include_neighbor_labels=False  — no oracle GT class names in neighbor prefix
#
# Run:
#   GPUS=4,5 bash /home/lingjie7/auto-research/projects/dlm-graph/examples/tmdlm/run_sft_pubmed_nonb_lora.sh
#
# Optional env vars:
#   GPUS=0,1
#   NUM_EPOCHS=10
...
INCLUDE_NEIGHBOR_LABELS="${INCLUDE_NEIGHBOR_LABELS:-False}"   # key difference
...
for i in "${!EXPERIMENTS[@]}"; do
  ...
  echo "[launch] dataset=${DATASET_NAME} gpu=${gpu_id} hops=${MAX_HOPS} topo_mask=${use_topology_mask} include_neighbor_labels=False output=${output_dir}"
  echo "[log] ${log_file}"
