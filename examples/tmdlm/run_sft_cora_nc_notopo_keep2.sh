#!/usr/bin/env bash
# Cora-only NC no-topology replication.
#
# Mirrors run_sft_cora_nc_topo_only_replicate.sh, but uses full causal
...
#SBATCH --job-name=cora-nc-notopo
#SBATCH --output=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/cora_nc_notopo_keep2_%j.log
#SBATCH --error=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/cora_nc_notopo_keep2_%j.log
...
RUN_TAG="cora_nc_notopo_$(date +%Y%m%d_%H%M)_mcdigit_d0_nonb_keep2"
run_name="tmdlm-llada-8b-cora-nc-2hop-notopo-mcdigit-d0-nonb-r64-ep10-${RUN_TAG}"
...
echo "[launch] recipe=cora-only topo=False mc_digit digit0 nonb cudnn_sdp=${TMDLM_ENABLE_CUDNN_SDP}"
