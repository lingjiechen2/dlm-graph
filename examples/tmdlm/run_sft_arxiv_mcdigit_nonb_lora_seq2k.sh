...
# TM-DLM SFT launcher: ogbn-arxiv, mc_digit, nonb (no neighbor labels), max_seq_len=2048, ~5 epochs.
# Variant of run_sft_arxiv_mcdigit_nonb_lora.sh that mirrors the products seq=2048 / 5-epoch
# recipe so wallclock fits comfortably within 1 day per setting. Runs topo + notopo in parallel
# on two GPUs.
#
# Usage:
#   GPUS=4,5 bash examples/tmdlm/run_sft_arxiv_mcdigit_nonb_lora_seq2k.sh
#
# Notes:
#   - 40 classes → labels "0".."39", up to 2 tokens.
#   - cls_loss_weight=0 since trainer.py:211 squeeze cannot handle multi-token answers
#     when cls_loss_weight>0 (same constraint as the seq4k launcher).
#   - At seq=2048 each neighbor budget is ~66 tokens (vs ~170 at seq=4096); arxiv abstract
#     median ~220, so this truncates more aggressively. Trade-off chosen for wallclock.
...
MAX_STEPS="${MAX_STEPS:-2085}"                  # ~5 epochs at 20k subsample / eff batch 48 (= 417 steps/epoch × 5)
MAX_TRAIN_SAMPLES="${MAX_TRAIN_SAMPLES:-20000}" # subsample arxiv train for faster tokenize/load
MAX_SEQ_LEN="${MAX_SEQ_LEN:-2048}"              # 2048 keeps wallclock ~3× faster than 4096

BASE_MODEL="${BASE_MODEL:-GSAI-ML/LLaDA-8B-Instruct}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/.models}"
RUN_TAG="${RUN_TAG:-arxiv_$(date +%Y%m%d)_mcdigit_nonb_seq2k}"
