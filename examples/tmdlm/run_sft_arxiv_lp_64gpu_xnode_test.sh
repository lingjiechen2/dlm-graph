#!/usr/bin/env bash
# ogbn-arxiv LP SFT multi-node test: 8 nodes × 8 H200 = 64 GPUs, 30 steps only.
# Mirrors the PubMed 64-GPU test pattern. Verifies cross-node DDP works on the
...
#
# Usage: sbatch examples/tmdlm/run_sft_arxiv_lp_64gpu_xnode_test.sh
#
...
#SBATCH --job-name=arxiv-lp-64gpu
#SBATCH --output=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/arxiv_lp_64gpu_test_%j.log
#SBATCH --error=/mnt/weka/home/lingjie.chen/model/dlm-graph/logs/arxiv_lp_64gpu_test_%j.log
...
RUN_TAG="arxiv_lp_64gpu_xnode_test_$(date +%Y%m%d_%H%M)"
