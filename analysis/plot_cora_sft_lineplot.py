"""
Line plot of Cora SFT checkpoint accuracy (logit + infill, topo + notopo).
Visual style loaded from analysis/plot_style.json.

Usage:
    python analysis/plot_cora_sft_lineplot.py \
        --jsonl_dir /tmp/dlm-graph-eval-jsonl \
        --run_tag llaga_20260427_aligned \
        --out analysis/figures/cora_sft_lineplot.png
"""
...
def main() -> None:
    ...
    pad = max(0.5, (hi_sft - lo_sft) * 0.15)
    ...
    ax.set_title("Cora", fontsize=style["title"]["fontsize"],
                 pad=style["title"]["pad"])
    lg = style["legend"]
