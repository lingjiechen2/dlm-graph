"""
Run PubMed final-checkpoint eval_infill step sweep with GPU polling.

This script keeps polling target GPUs and launches tasks only when a target GPU
becomes empty. It evaluates checkpoint-final for both topo/notopo runs with
steps in {1, 2, 4, 8}, while keeping all other eval_infill settings unchanged.

Run:
    source ~/.zshrc
    conda activate /home/lingjie7/anaconda3/envs/dllm
    python /home/lingjie7/auto-research/projects/dlm-graph/scripts/run_pubmed_final_infill_steps_wait_gpu26.py
"""
...
def discover_final_checkpoint(topo_tag: str) -> Path:
    pattern = (
        f"tmdlm-llada-8b-pubmed-2hop-{topo_tag}-"
...
def get_busy_gpus() -> set[int]:
    ...
    for line in out.strip().splitlines():
        ...
        uuid = parts[0]
        pid_s = parts[1]
        # Ignore stale records (e.g., nvidia-smi shows "[Not Found]" process).
...
def main() -> None:
    ...
    out_root = SUMMARIES_ROOT / f"pubmed_final_infill_steps_gpu26_wait_{now}"
    ...
    # Resume-friendly: skip tasks that already have non-empty jsonl outputs.
    pending: list[Task] = []
    for task in tasks:
        existing_jsonl = jsonl_dir / f"{task.exp_name}.jsonl"
        if existing_jsonl.exists() and existing_jsonl.stat().st_size > 0:
            print(f"[skip] existing result: {task.exp_name}", flush=True)
    ...
    while queue or running:
        ...
        while queue and free_gpus:
            ...
            cmd = build_cmd(task, jsonl_path=jsonl_path)
            env = os.environ.copy()
    ...
    finished.sort(key=lambda r: (r["topo_tag"], r["steps"]))
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(finished)

    md_path = out_root / "summary.md"
    md_lines = [
        "# PubMed Final Checkpoint Infill Step Sweep",
        "",
        f"- Timestamp: `{now}`",
