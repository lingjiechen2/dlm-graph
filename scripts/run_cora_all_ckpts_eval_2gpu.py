"""
Run all Cora checkpoints with both eval_logit and eval_infill on 2 GPUs.

Run:
    source ~/.zshrc
    conda activate /home/lingjie7/anaconda3/envs/dllm
    python /home/lingjie7/auto-research/projects/dlm-graph/scripts/run_cora_all_ckpts_eval_2gpu.py
"""
...
class Task:
    ...
    def exp_name(self) -> str:
        return (
            f"cora_{self.eval_type}_{self.ckpt.run_dir_name}_{self.ckpt.checkpoint_name}"
        )
...
def parse_ckpt_spec(path: Path) -> CkptSpec | None:
    ...
    if hops_token == "na":
        ...
        max_hops = int(hops_token.replace("hop", ""))
    use_topology_mask = topo_token == "topo"
    topo_tag = "topo1" if use_topology_mask else "topo0"
...
def main() -> None:
    ...
    run_root = summary_root / f"cora_all_ckpts_eval_{now}"
    jsonl_dir = run_root / "jsonl"
    stdout_dir = run_root / "stdout"
    jsonl_dir.mkdir(parents=True, exist_ok=True)
    stdout_dir.mkdir(parents=True, exist_ok=True)

    ckpt_paths = sorted(models_root.glob("tmdlm-llada-8b-cora-*/checkpoint-*"))
    specs: list[CkptSpec] = []
    for p in ckpt_paths:
        if not p.is_dir():
            continue
    ...
    finished.sort(key=lambda r: (r["run_dir_name"], r["checkpoint_name"], r["eval_type"]))
    ...
    md_lines = [
        "# Cora All Checkpoints Eval Summary",
        "",
        f"- Timestamp: `{now}`",
